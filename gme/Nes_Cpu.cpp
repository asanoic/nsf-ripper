...
#define GET_ADDR() ((uint32_t)*(uint16_t*)(instr))
...
#define BRANCH(cond)                                  \
    ...
    case 0x6C: { // JMP (ind)
        data = GET_ADDR();
        assert(unsigned(data - 0x2000) >= 0x4000); // ensure it's outside I/O space
...
interrupt :
{
    ...
    pc = *(uint16_t*)(&READ_PROG(0xFFFA) + result_);
