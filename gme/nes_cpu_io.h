...
void Nsf_Emu::cpu_write(nes_addr_t addr, int data) {
    ...
    if (bank < bank_count) {
        long offset = rom.mask_addr(data * (long)bank_size);
        if (offset >= rom.size())
...
#define CPU_READ(cpu, addr, time) ((Nsf_Emu&)*cpu).cpu_read(addr)
#define CPU_WRITE(cpu, addr, data, time) ((Nsf_Emu&)*cpu).cpu_write(addr, data)
