...
static void gen_sinc(float* out, int count, double oversample, double treble, double cutoff) {
    ...
    for (int i = 0; i < count; i++) {
        ...
        // Fs/2*cutoff to Fs/2, longarithmic rolloff
        ...
        if (den > 1e-13) {
            double num = (cos(angle_maxh - angle) * rolloff - cos(angle_maxh)) * pow_a_n - cos(angle_maxh_mid - angle) * rolloff + cos(angle_maxh_mid);
...
long Blip_Buffer::read_samples(short* BLIP_RESTRICT out, long max_samples, int stereo) {
...
void Blip_Buffer::mix_samples(short const* in, long count) {
    if (buffer_size_ == silent_buf_size) {
        assert(0);
        return;
    }
