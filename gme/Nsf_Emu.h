...
public:
    ...
    const char* load(header_t const& h, Data_Reader& in) // use Remaining_Reader
    {
...
protected:
    const char* track_info_(track_info_t*, int track) const;
    const char* load_(Data_Reader&);
    const char* start_track_(int);
    const char* run_clocks(blip_time_t&, int);
