...
public:
    ...
    virtual const char* read(void*, long count);

    // Number of bytes remaining until end of file
    virtual long remain() const = 0;

    // Read and discard count bytes
    virtual const char* skip(long count);

public:
    Data_Reader() {
    }
    typedef const char* error_t; // deprecated
private:
    // noncopyable
...
public:
    ...
    // Go to new position
    virtual const char* seek(long) = 0;
...
public:
    ...
    ~Std_File_Reader();
    long size() const;
    const char* read(void*, long);
    long read_avail(void*, long);
    long tell() const;
    const char* seek(long);
