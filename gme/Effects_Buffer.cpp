...
void Effects_Buffer::set_depth(double d) {
    ...
    if (f > 0.5) f = 0.5; // TODO: more linear reduction of extreme reverb/echo
...
Effects_Buffer::Effects_Buffer(bool center_only) :
    Multi_Buffer(2) {
...
const char* Effects_Buffer::set_sample_rate(long rate, int msec) {
    if (!echo_buf.size())
        echo_buf.resize(echo_size);

    if (!reverb_buf.size())
        reverb_buf.resize(reverb_size);

    for (int i = 0; i < buf_count; i++)
        if (const char* res = bufs[i].set_sample_rate(rate, msec)) return res;
...
long Effects_Buffer::read_samples(short* out, long total_samples) {
    assert(total_samples % 2 == 0); // count must be even
...
void Effects_Buffer::mix_mono(short* out_, long count) {
    short* BLIP_RESTRICT out = out_;
    int const bass = BLIP_READER_BASS(bufs[0]);
    BLIP_READER_BEGIN(c, bufs[0]);

    // unrolled loop
    for (long n = count >> 1; n; --n) {
        long cs0 = BLIP_READER_READ(c);
        BLIP_READER_NEXT(c, bass);

        long cs1 = BLIP_READER_READ(c);
        BLIP_READER_NEXT(c, bass);
...
void Effects_Buffer::mix_stereo(short* out_, long count) {
    short* BLIP_RESTRICT out = out_;
    int const bass = BLIP_READER_BASS(bufs[0]);
    BLIP_READER_BEGIN(c, bufs[0]);
...
void Effects_Buffer::mix_mono_enhanced(short* out_, long count) {
    short* BLIP_RESTRICT out = out_;
    ...
    short* const reverb_buf = this->reverb_buf.data();
    short* const echo_buf = this->echo_buf.data();
    ...
    while (count--) {
        ...
        int new_reverb_l = FMUL(sum1_s, chans.pan_1_levels[0]) + FMUL(sum2_s, chans.pan_2_levels[0]) + reverb_buf[(reverb_pos + chans.reverb_delay_l) & reverb_mask];

        int new_reverb_r = FMUL(sum1_s, chans.pan_1_levels[1]) + FMUL(sum2_s, chans.pan_2_levels[1]) + reverb_buf[(reverb_pos + chans.reverb_delay_r) & reverb_mask];

        fixed_t reverb_level = chans.reverb_level;
        reverb_buf[reverb_pos] = (short)FMUL(new_reverb_l, reverb_level);
        reverb_buf[reverb_pos + 1] = (short)FMUL(new_reverb_r, reverb_level);
...
void Effects_Buffer::mix_enhanced(short* out_, long count) {
    short* BLIP_RESTRICT out = out_;
    int const bass = BLIP_READER_BASS(bufs[2]);
    BLIP_READER_BEGIN(center, bufs[2]);
    ...
    while (count--) {
        ...
        int new_reverb_l = FMUL(sum1_s, chans.pan_1_levels[0]) + FMUL(sum2_s, chans.pan_2_levels[0]) + BLIP_READER_READ(l1) + reverb_buf[(reverb_pos + chans.reverb_delay_l) & reverb_mask];

        int new_reverb_r = FMUL(sum1_s, chans.pan_1_levels[1]) + FMUL(sum2_s, chans.pan_2_levels[1]) + BLIP_READER_READ(r1) + reverb_buf[(reverb_pos + chans.reverb_delay_r) & reverb_mask];
        ...
        int left = new_reverb_l + sum3_s + BLIP_READER_READ(l2) + FMUL(chans.echo_level, echo_buf[(echo_pos + chans.echo_delay_l) & echo_mask]);
        int right = new_reverb_r + sum3_s + BLIP_READER_READ(r2) + FMUL(chans.echo_level, echo_buf[(echo_pos + chans.echo_delay_r) & echo_mask]);
