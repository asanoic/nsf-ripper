...
public:
    ...
    const char* load_mem(void const* data, long size);

    // Load an m3u playlist. Must be done after loading main music file.
    const char* load_m3u(const char* path);
    const char* load_m3u(Data_Reader& in);
    ...
    const char* track_info(track_info_t* out, int track) const;
...
protected:
    ...
    void set_track_count(int n) {
        track_count_ = raw_track_count_ = n;
    }
    ...
    const char* load_remaining_(void const* header, long header_size, Data_Reader& remaining);

    // Overridable
    virtual void unload();                                       // called before loading file and if loading fails
    virtual const char* load_(Data_Reader&);                    // default loads then calls load_mem_()
    virtual const char* load_mem_(byte const* data, long size); // use data in memory
    virtual const char* track_info_(track_info_t* out, int track) const = 0;
...
public:
    const char* remap_track_(int* track_io) const; // need by Music_Emu
private:
    // noncopyable
    ...
    vector<byte> file_data; // only if loaded into memory using default load

    const char* load_m3u_(const char*);
    const char* post_load(const char* err);
...
inline gme_type_t Gme_File::type() const {
    return type_;
}
inline int Gme_File::error_count() const {
    return warning_ != 0;
}
inline int Gme_File::track_count() const {
    return track_count_;
}
