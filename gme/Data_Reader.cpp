...
#define RETURN_VALIDITY_CHECK(cond)                   \
    do {                                              \
        if (!(cond)) return "Corrupt file"; \
    } while (0)

const char* Data_Reader::read(void* p, long s) {
...
const char* Data_Reader::skip(long count) {
    ...
    while (count) {
        ...
        if (n > count)
            n = count;
        count -= n;
        if (const char* res = read(buf, n)) return res;
    }
    return 0;
}
...
const char* File_Reader::skip(long n) {
...
long Remaining_Reader::remain() const {
    return header_end - header + in->remain();
}
...
const char* Remaining_Reader::read(void* out, long count) {
    count = max(0l, count);
    long first = read_first(out, count);
    long second = max(0l, count - first);
    if (!second)
...
const char* Mem_File_Reader::seek(long n) {
    RETURN_VALIDITY_CHECK(n >= 0);
    if (n > size_)
        return eof_error;
    pos = n;
...
const char* Callback_Reader::read(void* out, long count) {
    RETURN_VALIDITY_CHECK(count >= 0);
    if (count > remain_)
        return eof_error;
...
const char* Std_File_Reader::read(void* p, long s) {
...
const char* Std_File_Reader::seek(long n) {
...
const char* Gzip_File_Reader::open(const char* path) {
    close();
...
const char* Gzip_File_Reader::seek(long n) {
