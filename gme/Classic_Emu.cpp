...
const char* Classic_Emu::set_sample_rate_(long rate) {
    if (!buf) {
        if (!stereo_buffer) stereo_buffer = new Stereo_Buffer;
        buf = stereo_buffer;
    }
...
const char* Classic_Emu::setup_buffer(long rate) {
    change_clock_rate(rate);
    if (const char* res = buf->set_channel_count(voice_count())) return res;
    set_equalizer(equalizer());
    buf_changed_count = buf->channels_changed_count();
    return 0;
}

const char* Classic_Emu::start_track_(int track) {
    if (const char* res = Music_Emu::start_track_(track)) return res;
    buf->clear();
    return 0;
}

const char* Classic_Emu::play_(long count, sample_t* out) {
    long remain = count;
    while (remain) {
        ...
        if (remain) {
            ...
            int msec = buf->length();
            blip_time_t clocks_emulated = (long)msec * clock_rate_ / 1000;
            if (const char* res = run_clocks(clocks_emulated, msec)) return res;
...
const char* Rom_Data_::load_rom_data_(Data_Reader& in,
    ...
    rom.resize(file_offset + file_size_ + pad_size);
    const char* err = in.read(rom.data() + file_offset, file_size_);
    if (err) {
        rom.clear();
        return err;
    }
    ...
    memset(rom.data(), fill, pad_size);
    memset(rom.data() + rom.size() - pad_size, fill, pad_size);
...
void Rom_Data_::set_addr_(long addr, int unit) {
    ...
    if (addr < 0)
        addr = 0;
    size_ = rounded;
    rom.resize(rounded - rom_addr + pad_extra);
}
