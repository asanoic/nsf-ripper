...
public:
    ...
    const char* sample_rate(long r) {
        return set_sample_rate(r);
    }
    const char* sample_rate(long r, int msec) {
...
class blip_eq_t {
public:
    // longarithmic rolloff to treble dB at half sampling rate. Negative values reduce
...
public:
    ...
    const char* set_sample_rate(long samples_per_sec, int msec_length);
    blip_time_t count_clocks(long count) const;
    void mix_samples(short const* buf, long count);
