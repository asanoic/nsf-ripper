...
protected:
    // Services
    enum { wave_type = 0x100,
           noise_type = 0x200,
           mixed_type = wave_type | noise_type };
    void set_voice_types(int const* t) {
        voice_types = t;
    }
    const char* setup_buffer(long clock_rate);
    ...
    virtual const char* start_track_(int track) = 0;
    virtual const char* run_clocks(blip_time_t& time_io, int msec) = 0;

protected:
    const char* set_sample_rate_(long sample_rate);
    void mute_voices_(int);
    void set_equalizer_(equalizer_t const&);
    const char* play_(long, sample_t*);
...
protected:
    ...
    long rom_addr;
    long mask;
    long size_; // TODO: eliminate

    const char* load_rom_data_(Data_Reader& in, int header_size, void* header_out,
...
public:
    ...
    const char* load(Data_Reader& in, int header_size, void* header_out, int fill) {
    ...
    long mask_addr(long addr) const {
#ifdef check
        check(addr <= mask);
#endif
        return addr & mask;
