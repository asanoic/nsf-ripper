...
const char* Mono_Buffer::set_sample_rate(long rate, int msec) {
    if (const char* res = buf.set_sample_rate(rate, msec)) return res;
...
long Stereo_Buffer::read_samples(short* out, long count) {
...
void Stereo_Buffer::mix_stereo(short* out_, long count) {
    ...
    for (; count; --count) {
        ...
        long l = c + BLIP_READER_READ(left);
        long r = c + BLIP_READER_READ(right);
        if ((int16_t)l != l)
            l = 0x7FFF - (l >> 24);
...
void Stereo_Buffer::mix_stereo_no_center(short* out_, long count) {
    short* BLIP_RESTRICT out = out_;
    int const bass = BLIP_READER_BASS(bufs[1]);
    BLIP_READER_BEGIN(left, bufs[1]);
    BLIP_READER_BEGIN(right, bufs[2]);
    ...
    for (; count; --count) {
        long l = BLIP_READER_READ(left);
        if ((int16_t)l != l)
            l = 0x7FFF - (l >> 24);

        long r = BLIP_READER_READ(right);
        if ((int16_t)r != r)
            r = 0x7FFF - (r >> 24);
...
void Stereo_Buffer::mix_mono(short* out_, long count) {
    short* BLIP_RESTRICT out = out_;
    int const bass = BLIP_READER_BASS(bufs[0]);
    BLIP_READER_BEGIN(center, bufs[0]);

    for (; count; --count) {
        long s = BLIP_READER_READ(center);
        if ((int16_t)s != s)
            s = 0x7FFF - (s >> 24);
