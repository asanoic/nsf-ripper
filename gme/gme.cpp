...
gme_err_t gme_identify_file(const char* path, gme_type_t* type_out) {
    *type_out = gme_identify_extension(path);
    ...
    if (!*type_out) {
        char header[4];
        GME_FILE_READER in;
        if (gme_err_t res = in.open(path)) return res;
        if (gme_err_t res = in.read(header, sizeof header)) return res;
...
#if !GME_DISABLE_STEREO_DEPTH
            if (type->flags_ & 1) {
                me->effects_buffer = new Effects_Buffer;
...
gme_err_t gme_load_file(Music_Emu* me, const char* path) {
    return me->load_file(path);
}
...
gme_err_t gme_track_info(Music_Emu const* me, gme_info_t** out, int track) {
    *out = NULL;
    ...
    gme_info_t_* info = new gme_info_t_;
...
void gme_free_info(gme_info_t* info) {
    delete (gme_info_t_*)info;
}

void gme_set_stereo_depth(Music_Emu* me, double depth) {
#if !GME_DISABLE_STEREO_DEPTH
    if (me->effects_buffer)
        ((Effects_Buffer*)(me->effects_buffer))->set_depth(depth);
#endif
}
...
void gme_set_user_data(Music_Emu* me, void* new_user_data) {
    me->set_user_data(new_user_data);
}
void gme_set_user_cleanup(Music_Emu* me, gme_user_cleanup_t func) {
    me->set_user_cleanup(func);
}

gme_err_t gme_start_track(Music_Emu* me, int index) {
    return me->start_track(index);
}
gme_err_t gme_play(Music_Emu* me, int n, short* p) {
    return me->play(n, p);
}
void gme_set_fade(Music_Emu* me, int start_msec) {
    me->set_fade(start_msec);
}
int gme_track_ended(Music_Emu const* me) {
    return me->track_ended();
}
int gme_tell(Music_Emu const* me) {
    return me->tell();
}
int gme_tell_samples(Music_Emu const* me) {
    return me->tell_samples();
}
gme_err_t gme_seek(Music_Emu* me, int msec) {
    return me->seek(msec);
}
gme_err_t gme_seek_samples(Music_Emu* me, int n) {
    return me->seek_samples(n);
}
int gme_voice_count(Music_Emu const* me) {
    return me->voice_count();
}
void gme_ignore_silence(Music_Emu* me, int disable) {
    me->ignore_silence(disable != 0);
}
void gme_set_tempo(Music_Emu* me, double t) {
    me->set_tempo(t);
}
void gme_mute_voice(Music_Emu* me, int index, int mute) {
    me->mute_voice(index, mute != 0);
}
void gme_mute_voices(Music_Emu* me, int mask) {
    me->mute_voices(mask);
}
void gme_enable_accuracy(Music_Emu* me, int enabled) {
    me->enable_accuracy(enabled);
}
void gme_clear_playlist(Music_Emu* me) {
    me->clear_playlist();
}
int gme_type_multitrack(gme_type_t t) {
    return t->track_count != 1;
}
