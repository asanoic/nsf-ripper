...
Music_Emu::equalizer_t const Music_Emu::tv_eq = Music_Emu::make_equalizer(-8.0, 180);
...
const char* Music_Emu::set_sample_rate(long rate) {
    assert(!sample_rate()); // sample rate can't be changed once set
    if (const char* res = set_sample_rate_(rate)) return res;
    buf.resize(buf_size);
    sample_rate_ = rate;
    return 0;
}
...
void Music_Emu::pre_load() {
    assert(sample_rate()); // set_sample_rate() must be called before loading a file
    Gme_File::pre_load();
}
...
void Music_Emu::mute_voice(int index, bool mute) {
    assert((unsigned)index < (unsigned)voice_count());
    int bit = 1 << index;
...
void Music_Emu::mute_voices(int mask) {
    assert(sample_rate()); // sample rate must be set first
    mute_mask_ = mask;
    mute_voices_(mask);
}
...
const char* Music_Emu::start_track(int track) {
    clear_track_vars();

    int remapped = track;
    if (const char* res = remap_track_(&remapped)) return res;
    current_track_ = track;
    if (const char* res = start_track_(remapped)) return res;
...
void Music_Emu::end_track_if_error(const char* err) {
    if (err) {
        emu_track_ended_ = true;
        set_warning(err);
    }
...
const char* Music_Emu::seek_samples(long time) {
    if (time < out_time)
        if (const char* res = start_track(current_track_)) return res;
    return skip(time - out_time);
}

const char* Music_Emu::seek(long msec) {
    return seek_samples(msec_to_samples(msec));
}

const char* Music_Emu::skip(long count) {
    assert(current_track() >= 0); // start_track() must have been called already
    out_time += count;
...
const char* Music_Emu::skip_(long count) {
    ...
    if (count > threshold) {
        ...
        while (count > threshold / 2 && !emu_track_ended_) {
            if (const char* res = play_(buf_size, buf.data())) return res;
            count -= buf_size;
        }
    ...
    while (count && !emu_track_ended_) {
        ...
        if (n > count)
            n = count;
        count -= n;
        if (const char* res = play_(n, buf.data())) return res;
    }
    return 0;
}
...
void Music_Emu::fill_buf() {
    assert(!buf_remain);
    if (!emu_track_ended_) {
        emu_play(buf_size, buf.data());
        long silence = count_silence(buf.data(), buf_size);
        if (silence < buf_size) {
...
const char* Music_Emu::play(long out_count, sample_t* out) {
    if (track_ended_) {
        ...
        if (buf_remain) {
            ...
            memcpy(&out[pos], buf.data() + (buf_size - buf_remain), n * sizeof *out);
            buf_remain -= n;
            pos += n;
        }
...
const char* Gme_Info_::set_sample_rate_(long) {
    return 0;
}
void Gme_Info_::pre_load() {
    Gme_File::pre_load();
} // skip Music_Emu
void Gme_Info_::post_load_() {
    Gme_File::post_load_();
} // skip Music_Emu
void Gme_Info_::set_equalizer_(equalizer_t const&) {
    assert(false);
}
void Gme_Info_::enable_accuracy_(bool) {
    assert(false);
}
void Gme_Info_::mute_voices_(int) {
    assert(false);
}
void Gme_Info_::set_tempo_(double) {
}
const char* Gme_Info_::start_track_(int) {
    return "Use full emulator for playback";
}
const char* Gme_Info_::play_(long, sample_t*) {
    return "Use full emulator for playback";
}
