...
Nsf_Emu::equalizer_t const Nsf_Emu::nes_eq = Music_Emu::make_equalizer(-1.0, 80);
Nsf_Emu::equalizer_t const Nsf_Emu::famicom_eq = Music_Emu::make_equalizer(-15.0, 80);
...
const char* Nsf_Emu::track_info_(track_info_t* out, int) const {
    copy_nsf_fields(header_, out);
    return 0;
}

static const char* check_nsf_header(void const* header) {
...
struct Nsf_File : Gme_Info_ {
    ...
    const char* load_(Data_Reader& in) {
        const char* err = in.read(&h, Nsf_Emu::header_size);
        if (err)
    ...
    const char* track_info_(track_info_t* out, int) const {
        copy_nsf_fields(h, out);
        return 0;
    }
};

static Music_Emu* new_nsf_emu() {
    return new Nsf_Emu;
}
static Music_Emu* new_nsf_file() {
    return new Nsf_File;
}
...
void Nsf_Emu::set_tempo_(double t) {
    unsigned playback_rate = *(uint16_t*)(header_.ntsc_speed);
    ...
    if (pal_only) {
        ...
        playback_rate = *(uint16_t*)(header_.pal_speed);
    }
...
#define APU_NAMES "Square 1", "Square 2", "Triangle", "Noise", "DMC"
...
#else
    {
        ...
        if (header_.chip_flags & vrc6_flag) {
            ...
            if (header_.chip_flags & namco_flag) {
                int const count = Nes_Apu::osc_count + Nes_Vrc6_Apu::osc_count + Nes_Namco_Apu::osc_count;
                static const char* const names[count] = {
                    APU_NAMES,
                    "Saw Wave", "Square 3", "Square 4",
                    "Wave 1", "Wave 2", "Wave 3", "Wave 4",
...
const char* Nsf_Emu::load_(Data_Reader& in) {
    assert(offsetof(header_t, unused[4]) == header_size);
    if (const char* res = rom.load(in, header_size, &header_, 0)) return res;

    set_track_count(header_.track_count);
    if (const char* res = check_nsf_header(&header_)) return res;

    if (header_.vers != 1)
        set_warning("Unknown file version");

    // sound and memory
    const char* err = init_sound();
    if (err)
        return err;

    // set up data
    nes_addr_t load_addr = *(uint16_t*)(header_.load_addr);
    init_addr = *(uint16_t*)(header_.init_addr);
    play_addr = *(uint16_t*)(header_.play_addr);
...
const char* Nsf_Emu::start_track_(int track) {
    if (const char* res = Classic_Emu::start_track_(track)) return res;
...
const char* Nsf_Emu::run_clocks(blip_time_t& duration, int) {
    set_time(0);
    while (time() < duration) {
